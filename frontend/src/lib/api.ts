...
export const authStore = {
  getToken: () => localStorage.getItem("spc_token"),
  setToken: (token: string) => localStorage.setItem("spc_token", token),
  clear: () => localStorage.removeItem("spc_token"),
  getUser: () => {
    const raw = localStorage.getItem("spc_user");
    return raw ? JSON.parse(raw) : null;
  },
  setUser: (user: unknown) => localStorage.setItem("spc_user", JSON.stringify(user)),
};
